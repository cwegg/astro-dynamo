...
"""This file contains some example targets. To understand how to use them
checkout the astro_dynamo_example.ipynb notebook"""
...
class SurfaceDensity(nn.Module):
    def __init__(self,
                 ...
                 physical: bool = False):
        """
        Target used to match a surface density. All targets should be torch
        nn.Module and must provide a forward method. This method tells torch
        how to compute a set of observables on the model. When these
        observables then appear in the loss function to be optimised (e.g. in a
        likelihood) then we can ask torch to optimise them.

        Args:
            r_range: The range of radii over which we wish to fit
            r_bins: The number of bins
            physical: Are we using kpc (True) or internal units (False)
        """
        ...
        self.register_buffer('area',
                             math.pi * (redge[1:] ** 2 - redge[:-1] ** 2))

    def forward(self, model: DynamicalModel) -> torch.Tensor:
        """
        Compute surface density observables on a model
        Args:
            model: the DynamicalModel to compute

        Returns:
            A tensor r_bins long
        """
        snap = model.snap
        ...
        mass_in_bin = torch.sparse.FloatTensor(i[gd].unsqueeze(0),
                                               snap.masses[gd],
    ...
    def rmid(self) -> torch.Tensor:
        ...
        Returns: The mid point of the bins on which we compute the density

        """
        return self.r_min + self.dr / 2 + self.dr * torch.arange(self.r_bins,
    ...
    def evalulate_function(self, surface_density: Callable[[torch.Tensor], torch.Tensor]) -> torch.Tensor:
        ...
        Args:
            surface_density: a function which returns the surface density
            when provided a disk cylindical radius

        Returns:
            The surface density observable in a length r_bins tensor
        """
...
class DoubleExponentialDisk(nn.Module):
    def __init__(self,
                 ...
                 physical: bool = False):
        """
        Target used to match a double exppnential disk. That is the disk is an
        exponential both vertically and in cylindrical radius.

        Args:
            r_range: The range of radii over which we wish to fit
            r_bins: The number of radial bins
            z_range: The range of z over which we wish to fit
            z_bins: The number of z bins
            physical: Are we using kpc (True) or internal units (False)
        """
    ...
    def forward(self, model: DynamicalModel) -> torch.Tensor:
        """
        Compute disk density observables on a model
        Args:
            model: the DynamicalModel to compute

        Returns:
            A tensor of size (r_bins, z_bins)
        """
        snap = model.snap
        r_cyl = (snap.positions[:, 0] ** 2 + snap.positions[:, 1] ** 2).sqrt()
        if self.physical:
            r_cyl *= model.d_scale
        i = ((r_cyl - self.r_min) / self.dr).floor().type(torch.long)
        j = ((snap.positions[:, 2] - self.z_min) / self.dz).floor().type(
            torch.long)
        gd = (i >= 0) & (i < self.r_bins) & (j >= 0) & (j < self.z_bins)
        mass_in_bin = torch.sparse.FloatTensor(torch.stack((i[gd], j[gd])),
                                               snap.masses[gd],
                                               size=(self.r_bins,
                                                     self.z_bins)).to_dense()
    ...
    def zmid(self) -> torch.Tensor:
        return self.z_min + self.dz / 2 + self.dz * torch.arange(self.z_bins,
                                                                 device=self.area.device,
                                                                 dtype=self.area.dtype)
...
class DiskKinematics(SurfaceDensity):
    ...
    def forward(self, model: DynamicalModel) -> torch.Tensor:
        ...
        kin = torch.stack((hist_vphi, hist_vphi_sig, hist_vr, hist_vr_sig,
                           hist_vz, hist_vz_sig))
        if self.physical:
            kin *= model.v_scale
        return kin
...
def convolve_distance_modulus_with_lf(histogram, lf_number,
                                      distance_mod_dim=-1):
...
class PositionMagnitude(nn.Module):
    def __init__(self, luminosity_function, l_range=(-90., 90.), n_l=90,
        ...
        self.register_buffer('luminosity_function',
                             torch.as_tensor(luminosity_function['number'],
                                             dtype=torch.float))
    ...
    def _check_apparent_mags(self, lf_mags):
        ...
        out = convolve_distance_modulus_with_lf(
            self.gridder.grid_data(torch.as_tensor([[0, 0, 0]])),
            self.luminosity_function)
        assert torch.allclose(self.apparent_mags,
                              self.gridder.cell_midpoints[2][-1] + lf_mags[0] +
                              self.gridder.dx[2] * torch.arange(out.shape[2]))

    def forward(self, model):
        l_b_mu = self.gridder.grid_data(model.l_b_mu, weights=model.masses,
                                        method='nearest')
        out = convolve_distance_modulus_with_lf(l_b_mu,
                                                self.luminosity_function)
        return out
