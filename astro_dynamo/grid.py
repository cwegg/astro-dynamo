...
class Grid(nn.Module):
    ...
    def forward(self, snap):
        return self.grid_data(snap.positions, weights=snap.masses,
                              method='nearest')
    ...
    def ingrid(self, positions: torch.Tensor, h: float = None) -> torch.Tensor:
        ...
        if h is None:
            ...
            return ((positions > self.min[None, :] - h * self.dx[None, :]) &
                    (positions < self.max[None, :] + h * self.dx[None, :])).all(
                dim=1)
...
class ForceGrid(Grid):
    def __init__(self,
                 grid_edges: Union[list, tuple, torch.Tensor] = torch.tensor(
                     (10., 10., 10.)),
    ...
    def update_density(self, positions: torch.Tensor,
                       weights: torch.Tensor = None,
                       method: str = 'nearest',
                       fractional_update: float = 1.0) -> None:
    ...
    def smoothing_pot(r: torch.Tensor, epsilon: float = 1.) -> torch.Tensor:
        ...
        i = (x < 1)
        pot[i] = (-1.4 + x[i] ** 2 * (
                    - 0.3 * x[i] ** 2 + 2. / 3 + 0.1 * x[i] ** 3)) / epsilon
        i = (x >= 1) & (x < 2)
        pot[i] = (-1.6 + 1.0 / (15.0 * x[i]) + x[i] ** 2 * (
                    4. / 3 - x[i] + 0.3 * x[i] ** 2 - x[i] ** 3 / 30)) / epsilon
        return pot
    ...
    def get_accelerations(self, positions: torch.Tensor) -> torch.Tensor:
        ...
        return torch.nn.functional.grid_sample(image, samples, mode='bilinear',
    ...
    def get_potential(self, positions: torch.Tensor) -> torch.Tensor:
        ...
        # torch.nn.functional.grid_sample is fast but frustrating to juggle the inputs. See the pytorch documentation
        samples = (positions / self.max).unsqueeze(0).unsqueeze(2).unsqueeze(3)
        image = self.pot.permute(2, 1,
                                 0)  # change to:      C x W x H x D - with C=1 for potential
        image = image.unsqueeze(0).unsqueeze(
            0)  # change to:  1 x C x W x H x D - with C=1 for potential
        return torch.nn.functional.grid_sample(image, samples, mode='bilinear',
                                               align_corners=True).squeeze().t()

    def grid_accelerations(self, positions: torch.Tensor = None,
                           weights: torch.Tensor = None,
                           method: str = 'nearest'):
        ...
        if self.greenfft is None or padrho.shape != self.greenfft.shape[:-1] or \
            ...
            x = torch.arange(-nx, nx, dtype=rho.dtype, device=rho.device) * \
                self.dx[0]
            y = torch.arange(-ny, ny, dtype=rho.dtype, device=rho.device) * \
                self.dx[1]
            z = torch.arange(-nz, nz, dtype=rho.dtype, device=rho.device) * \
                self.dx[2]
            self.greenfft = torch.rfft(
                ForceGrid.smoothing_pot((x[:, None, None] ** 2 +
                                         y[None, :, None] ** 2 +
                                         z[None, None, :] ** 2).sqrt(),
        ...
        for dim in (0, 1, 2):
            self.acc[..., dim] = -self.__diff(self.pot, dim,
                                              d=self.dx[dim].item())
