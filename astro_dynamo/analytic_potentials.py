...
class SpheroidalPotential:
    ...
    def f_r_cyl(self, r_cyl, z, rel_tol=None, *args, **kwargs):
        """Return the force in cylindical R direction at (r_cyl, z)"""
        return self._f_compute(r_cyl, z, rel_tol, direction='r_cyl', *args, **kwargs)

    def f_z(self, r_cyl, z, rel_tol=None, *args, **kwargs):
        """Return the force in the z-direction at (r_cyl, z)"""
        return self._f_compute(r_cyl, z, rel_tol, direction='z', *args, **kwargs)
...
def fit_q_slice_to_snapshot(snap):
    """Fit a flattening to a particle snapshot. We assume that the density varies like \rho \propto m**gamma with
