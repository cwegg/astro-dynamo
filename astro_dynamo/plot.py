...
def plot_snap_projections(model: Union[astro_dynamo.model.DynamicalModel, astro_dynamo.snap.SnapShot],
                          ...
                          vmin: float = 1e-5,
                          vmax: float = 1e-2,
                          cmap: Union[str, matplotlib.colors.Colormap] = plt.cm.get_cmap(
    ...
    if isinstance(model, astro_dynamo.model.MilkyWayModel):
        snap, d_scale = model.snap, model.d_scale
    ...
    for ax, projection, projection_label in zip(axs.flatten(), projections, projection_labels):
...
def plot_disk_kinematics(model: astro_dynamo.model.DynamicalModel,
                         axs: Sequence[SubplotBase] = None) -> matplotlib.figure.Figure:
    ...
    try:
        disk_kinematics_obj = next(target for target in model.targets
                                   if isinstance(target, astro_dynamo.targets.DiskKinematics))
    except:
    ...
    for linestyle, kin, kin_err in zip(('-', '-.'),
                                       ...
                                       (None,)):
        if kin_err is None:
            axs[0].plot(r, kin[1, :].detach().cpu(), 'r', linestyle=linestyle, label='sig vphi')
            axs[0].plot(r, kin[3, :].detach().cpu(), 'g', linestyle=linestyle, label='sig vr')
            axs[0].plot(r, kin[5, :].detach().cpu(), 'b', linestyle=linestyle, label='sig vz')
        else:
    ...
    for linestyle, kin, kin_err in zip(('-', '-.'),
                                       ...
                                       (None,)):
        if kin_err is None:
            axs[1].plot(r, kin[0, :].detach().cpu(), 'r', linestyle=linestyle, label='mean vphi')
            axs[1].plot(r, kin[2, :].detach().cpu(), 'g', linestyle=linestyle, label='mean vr')
            axs[1].plot(r, kin[4, :].detach().cpu(), 'b', linestyle=linestyle, label='mean vz')
        else:
...
def plot_surface_density_profile(model: astro_dynamo.model.DynamicalModel,
    ...
    ax.semilogy(surface_density_obj.rmid.cpu(), surface_density_obj(model).detach().cpu(), label='Model')
    if target_values is not None:
        ax.semilogy(surface_density_obj.rmid.cpu(), target_values.detach().cpu(), label='Target')
    ax.set_xlabel('r')
