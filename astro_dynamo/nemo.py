...
You then need to set the environment variable NEMO_LOCATION to your install location

Calling Nemo can be a bit tempremental - the executables are dynamically linked, and the libraries
and executables are all in their own environment. We try to handle this, but if it fails then first
...
def getgravity(particles, positions, verbose=False):
    ...
    Outputs:
        (Fx,Fy,Fz,potential) at each postion. return in internal units i.e. assuming G=1.
    """
