...
import pytest
import torch
from astro_dynamo.model import DynamicalModel, MilkyWayModel
...
def mwmodel():
    ...
    model = MilkyWayModel(snap=snap, potentials=[], targets=[], v_scale=1.0, d_scale=1.0,r_0=8.0,z_0=0.1).double()
    return model
...
def test_l_b_mu_bar_endr():
    ...
    snap = SnapShot(positions=torch.as_tensor([[-bar_len, 0.0, 0.0]]),
                    velocities=torch.as_tensor([[0.0, 0.0, 0.0]]),
                    masses=torch.as_tensor([1.0])).double()
    model = MilkyWayModel(snap=snap, potentials=[], targets=[], v_scale=1.0, d_scale=1.0, r_0=r_0, z_0=0.0,
                          bar_angle=bar_angle).double()
    d = np.sqrt(bar_len**2 + r_0**2 - 2*bar_len*r_0*np.cos(np.pi*bar_angle/180))
    l = 180*np.arcsin(bar_len/d*np.sin(np.pi*bar_angle/180))/np.pi
    bar_end = [[l,0.0,5*np.log10(100*d)]]
...
def test_get_masses():
    ...
    model = MilkyWayModel(snap=snap, potentials=[], targets=[], v_scale=v_scale, d_scale=d_scale).double().eval()
    np.testing.assert_array_almost_equal(model.masses.detach()[0], d_scale * 1e3 * v_scale ** 2 / 4.302E-3)
