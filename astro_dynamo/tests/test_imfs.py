...
def test_integral_limits(simple_power_law_imf):
    np.testing.assert_array_almost_equal(
        simple_power_law_imf.mass_integral([0, np.inf]), [0, 1.0])


def test_number(simple_power_law_imf):
    np.testing.assert_array_almost_equal(simple_power_law_imf.number([0.5]),
                                         [8 / 3])


def test_integral(simple_power_law_imf):
    np.testing.assert_array_almost_equal(
        simple_power_law_imf.number_integral([0.5]), [4 / 3])
