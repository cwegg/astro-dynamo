...
from astro_dynamo.plot import plot_disk_kinematics, plot_snap_projections, \
    plot_surface_density_profile
...
def mwmodel():
    ...
    model = MilkyWayModel(snap=snap, potentials=[], targets=targets,
                          v_scale=1.0, d_scale=1.0, r_0=8.0, z_0=0.0)
    return model
...
def test_plot_surface_density_profile(mwmodel):
    ...
    assert isinstance(plot_surface_density_profile(mwmodel, ax, mwmodel()[1]),
